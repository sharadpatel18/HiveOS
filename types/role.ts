// types/role.ts

/**
 * Single source of truth for all user roles.
 * Never use raw strings for roles anywhere else.
 */

export const ROLES = {
  USER: "USER",
  RECRUITER: "RECRUITER",
  FOUNDER: "FOUNDER",
  EMPLOYEE: "EMPLOYEE",
  SUPERADMIN: "SUPERADMIN",
  TEAMLEAD: "TEAMLEAD",
  MANAGER: "MANAGER",
} as const;

/**
 * Union type of all roles
 */
export type Role = (typeof ROLES)[keyof typeof ROLES];

/**
 * Role hierarchy (higher = more power)
 * Useful for permission checks & promotions
 */
export const ROLE_HIERARCHY: Record<Role, number> = {
  USER: 1,
  EMPLOYEE: 2,
  RECRUITER: 3,
  FOUNDER: 4,
  SUPERADMIN: 5,
  TEAMLEAD: 6,
  MANAGER: 7,
};

/**
 * Permission groups (recommended over direct role checks)
 */
export const ROLE_GROUPS = {
  // Company lifecycle
  CAN_CREATE_COMPANY: [ROLES.USER],
  CAN_MANAGE_COMPANY: [ROLES.FOUNDER],

  // Hiring — matches your flow exactly
  CAN_POST_JOBS: [ROLES.FOUNDER, ROLES.RECRUITER],
  CAN_INVITE_RECRUITER: [ROLES.FOUNDER],
  CAN_INVITE_MANAGER: [ROLES.FOUNDER, ROLES.RECRUITER],
  CAN_INVITE_TEAMLEAD: [ROLES.FOUNDER, ROLES.RECRUITER, ROLES.MANAGER],
  CAN_INVITE_EMPLOYEES: [
    ROLES.FOUNDER,
    ROLES.RECRUITER,
    ROLES.MANAGER,
    ROLES.TEAMLEAD,
  ],
  //teams

  CAN_CREATE_TEAM: [ROLES.FOUNDER, ROLES.MANAGER, ROLES.TEAMLEAD],
  CAN_MANAGE_TEAM: [ROLES.FOUNDER, ROLES.MANAGER, ROLES.TEAMLEAD],
  // System-level
  CAN_ACCESS_SYSTEM: [ROLES.SUPERADMIN],
} as const;

/**
 * Helper utilities
 */

export function hasRole(
  userRole: Role,
  allowedRoles: readonly Role[],
): boolean {
  return allowedRoles.includes(userRole);
}

export function hasHigherOrEqualRole(
  userRole: Role,
  requiredRole: Role,
): boolean {
  return ROLE_HIERARCHY[userRole] >= ROLE_HIERARCHY[requiredRole];
}
